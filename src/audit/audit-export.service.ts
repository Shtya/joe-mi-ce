...
export class AuditExportService {
  ...
  private readonly discountReasonTranslations: Record<
    DiscountReason,
    { en: string; ar: string }
  > = {
  ...
  private translateDiscountReason(
    reason: DiscountReason | string | null,
    language: "en" | "ar" = "en",
  ): string {
    if (!reason) return "";
    ...
    if (typeof reason === "string") {
      ...
      if (enumKey) {
        return (
          this.discountReasonTranslations[reason as DiscountReason]?.[
            language
          ] || reason
        );
      }
  ...
  async exportToExcel(query: any, req: any): Promise<Buffer> {
    ...
    qb.andWhere("project.id = :pid", {
      pid: user.project?.id ?? user.project_id,
    });
    ...
    // Extra filters
    if (query.branch_id) {
      qb.andWhere("branch.id = :branch", { branch: query.branch_id });
    }
    if (query.promoter_id) {
      qb.andWhere("promoter.id = :promoter", { promoter: query.promoter_id });
    }
    if (query.product_id) {
      qb.andWhere("product.id = :product", { product: query.product_id });
    }
    if (query.status) {
    ...
    if (query.origin) {
      ...
      const originQuery =
        Object.keys(this.countryTranslations).find(
          (key) =>
            this.countryTranslations[key].en === query.origin ||
    ...
    if (query.brand_id) {
      qb.andWhere("brand.id = :brandId", { brandId: query.brand_id });
    }
    if (query.category_id) {
      qb.andWhere("category.id = :catId", { catId: query.category_id });
    }
    if (query.brand_name) {
      qb.andWhere("brand.name ILIKE :bname", {
        bname: `%${query.brand_name}%`,
      });
    }
    if (query.category_name) {
      qb.andWhere("category.name ILIKE :cname", {
        cname: `%${query.category_name}%`,
      });
    }

    // Sorting
    qb.orderBy("audit.audit_date", "DESC").addOrderBy(
      "audit.created_at",
      "DESC",
    );
  ...
  getTranslatedCountries(): Array<{
    value: string;
    label_en: string;
    label_ar: string;
  }> {
    const countries = Object.entries(this.countryTranslations).map(
      ([key, translation]) => ({
  ...
  getTranslatedDiscountReasons(): Array<{
    value: DiscountReason;
    label_en: string;
    label_ar: string;
  }> {
    return Object.entries(this.discountReasonTranslations).map(
      ([key, translation]) => ({
