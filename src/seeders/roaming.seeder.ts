...
export const seedRoaming = async (dataSource: DataSource) => {
  ...
  for (const project of projects) {
    ...
    const hasRoaming = project.chains.some((c) => c.name === "Roaming");

    if (hasRoaming) {
      console.log(
        `Project ${project.name} already has a roaming chain. Skipping.`,
      );
      continue;
    }
    ...
    console.log(
      `Updating roaming=true for chain: ${newChain.name} in project: ${project.name}`,
    );
