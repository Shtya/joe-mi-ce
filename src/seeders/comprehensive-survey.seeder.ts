...
export const seedComprehensiveSurveys = async (dataSource: DataSource) => {
  ...
  if (!user) {
    console.warn(
      `⚠️ User with ID ${userId} not found. Skipping survey seeding.`,
    );
    return;
  }
  if (!project) {
    console.warn(
      `⚠️ Project with ID ${projectId} not found. Skipping survey seeding.`,
    );
    return;
  }
  ...
  for (const sData of surveysData) {
    ...
    if (existingSurvey) {
      console.log(
        `ℹ️ Survey "${sData.name}" already exists for this project. Skipping.`,
      );
      continue;
    }
    ...
    console.log(
      `✅ Seeded survey: ${sData.name} with ${questions.length} questions (all types included)`,
    );
  }
