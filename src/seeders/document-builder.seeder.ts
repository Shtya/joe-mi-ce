...
export const seedDocumentBuilder = async (dataSource: DataSource) => {
    ...
    elements: [
      ...
      },
      {
        ...
        tableData: {
          rows: 1,
          cols: 4,
          cells: [["ساكو", "مصرى", "6555988888", "احمد محمد فوزى"]],
          columnFields: [
            "work_location",
          ...
          columnLabels: [null, null, null, null, null, null, null],
  ...
  const elementsData = payload.elements.map((el) => {
    ...
    if ("fieldDisplayType" in el) {
      elem.fieldDisplayType = (el as any).fieldDisplayType;
    }
    if ("fieldLabelStyle" in el) {
      elem.fieldLabelStyle = (el as any).fieldLabelStyle;
    }
    if ("fieldValueStyle" in el) {
      elem.fieldValueStyle = (el as any).fieldValueStyle;
    }
    if ("tableData" in el) elem.tableData = (el as any).tableData;
    if ("lineStyle" in el) elem.lineStyle = (el as any).lineStyle;
    if ("lineOrientation" in el) {
      elem.lineOrientation = (el as any).lineOrientation;
    }
  ...
  console.log(
    `✅ Seeded Document (ID: ${savedDoc.id}) and ${elementsData.length} elements`,
  );
...
if (require.main === module) {
  ...
  console.log(
    "DB Config:",
    JSON.stringify({ ...dbConfig, password: "***" }, null, 2),
  );
