...
export class ReportsService {
  ...
  async generateMonthlyReport(
    ...
    if (options?.brandFilter) {
      salesQuery.andWhere("LOWER(brand.name) = :brandName", {
        brandName: options.brandFilter.toLowerCase(),
      });
    }
    ...
    const filename = `${options?.filenamePrefix || "monthly_report"}_${executionDateStr}.xlsx`;
  ...
  async generateDreameMonthlyReport(
    ...
    return this.generateMonthlyReport(givenDate, {
      brandFilter: "dreame",
      filenamePrefix: "dreame_monthly_report",
    });
  }
