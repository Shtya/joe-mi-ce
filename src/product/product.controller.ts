...
export class ProductController {
  ...
  async downloadTemplate(@Req() req: any, @Res() res: Response) {
    try {
      ...
      const buffer =
        await this.productService.generateImportTemplate(projectId);
      res.setHeader("Content-Type", "text/csv");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=product-import-template-${projectId}.csv`,
      );
      res.send(buffer);
    } catch (error) {
    ...
    try {
      ...
      if (file.mimetype.includes("csv")) {
        ...
        worksheet.getRow(1).eachCell((cell, colNumber) => {
          headers[colNumber - 1] =
            cell.value?.toString().trim().toLowerCase().replace(/\s+/g, "_") ||
            "";
        });
        ...
        products: products
          .map((row) => {
            ...
            return {
              ...
              category_name: mapColumn([
                "category_name",
                "category",
                "product_category",
              ]),
              brand_name: mapColumn(["brand_name", "brand", "manufacturer"]),
              origin_country: mapColumn([
                "origin_country",
                "country",
                "made_in",
              ]),
              quantity: parseInt(
                mapColumn(["quantity", "stock", "stock_quantity"]) || "0",
              ),
              all_branches: ["true", "1", "yes"].includes(
                (
                  mapColumn(["all_branches", "all_branch", "allbranches"]) || ""
                ).toLowerCase(),
              ),
   ...
   */
  private findHeaderRow(rows: any[][]): {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      for (const header of normalizedHeaders) {
        if (
          knownHeaders.some((k) => header.includes(k) || k.includes(header))
        ) {
          matchCount++;
        }
      }
  ...
  async importAndUpdateProducts(
    ...
    try {
      ...
      if (file.mimetype === "text/csv") {
        ...
        await this.processExcelInBatches(
          filePath,
          projectId,
          BATCH_SIZE,
          result,
      ...
      console.log(
        `Import completed: Created: ${result.created}, Updated: ${result.updated}, Failed: ${result.failed}`,
      );
      return result;
    } catch (err) {
  ...
  private async processCSVInBatches(
    ...
    try {
      ...
      console.log(
        `CSV Headers detected at row ${headerRowIndex + 1}: ${headers.join(", ")}`,
      );
  ...
  private async processExcelInBatches(
    ...
    try {
      if (filePath.endsWith(".xls")) {
        ...
        console.log(
          `Excel Headers detected at row ${headerRowIndex + 1}: ${headers.join(", ")}`,
        );
        ...
        for await (const worksheetReader of workbook) {
          if (worksheetProcessed) {
            console.warn(
              "Multiple worksheets found, only processing first worksheet",
            );
            break;
          }
          ...
          for await (const row of worksheetReader) {
            ...
            if (!isHeaderFound) {
              ...
              if (rowBuffer.length >= MAX_BUFFER) {
                const { headerRowIndex: foundIndex, headers: foundHeaders } =
                  this.findHeaderRow(rowBuffer.map((r) => r.values));
                ...
                isHeaderFound = true;
                console.log(
                  `Excel Headers detected at row ${rowBuffer[headerRowIndex].index}: ${headers.join(", ")}`,
                );

                // Process rows in buffer that are AFTER the header
                for (let i = headerRowIndex + 1; i < rowBuffer.length; i++) {
                  await this.processRowFromValues(
                    rowBuffer[i].values,
                    headers,
                    rowBuffer[i].index,
                    batch,
          ...
          if (!isHeaderFound && rowBuffer.length > 0) {
            ...
            headers = foundHeaders;
            console.log(
              `Excel Headers detected at row ${rowBuffer[foundIndex].index}: ${headers.join(", ")}`,
            );
  ...
  private cleanRowValues(row: any): void {
    ...
    if (
      row.all_branches !== undefined &&
      row.all_branches !== null &&
      row.all_branches !== ""
    ) {
      const allBranchesStr = String(row.all_branches).toLowerCase().trim();
      row.all_branches = ["true", "1", "yes", "oui", "vrai"].includes(
        allBranchesStr,
      );
    } else {
      // Default to true
    ...
    if (row.saco_sku) {
      ...
      row.saco_sku = ["not actv", "not active", "inactive", "na"].includes(
        sacoSkuStr,
      )
        ? null
        : row.saco_sku;
  ...
  private async processBatch(
    ...
    try {
      const batchResult =
        await this.productService.importAndUpdateProductsBatch(
          batch.map((b) => b.data),
          projectId,
          batch.map((b) => b.index),
        );
      ...
      // Log progress
      console.log(
        `Batch processed: ${batch.length} rows. Progress: ${result.processed}/${result.totalRows || "?"}`,
      );
      ...
      result.errors.push(
        `Batch failed at rows ${batch[0]?.index}-${batch[batch.length - 1]?.index}: ${error.message}`,
      );
    }
  }
}
