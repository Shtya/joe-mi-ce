...
const run = async () => {
  ...
  const locationRepo = dataSource.getRepository(PromoterLocation);
  const locationLogRepo = dataSource.getRepository(LocationLog);
  ...
  if (
    supervisorNotifEn &&
    supervisorNotifEn.title === "New check-in on your branch"
  ) {
    console.log("✅ Supervisor Notification (EN): OK");
  } else {
    console.error(
      `❌ Supervisor Notification (EN): FAIL. Got: ${supervisorNotifEn?.title}`,
    );
  }

  if (promoterNotifEn && promoterNotifEn.title === "Check-in Successful") {
    console.log("✅ Promoter Notification (EN): OK");
  } else {
    console.error(
      `❌ Promoter Notification (EN): FAIL. Got: ${promoterNotifEn?.title}`,
    );
  }
  ...
  const supervisorNotifAr = notifsAr.find(
    (n) =>
      n.user.id === supervisor!.id &&
      n.type === NotificationType.JOURNEY_CHECKOUT,
  );
  const promoterNotifAr = notifsAr.find(
    (n) =>
      n.user.id === promoter!.id &&
      n.type === NotificationType.JOURNEY_CHECKOUT,
  );

  if (
    supervisorNotifAr &&
    supervisorNotifAr.title === "تم تسجيل الخروج في فرعك"
  ) {
    console.log("✅ Supervisor Notification (AR): OK");
  } else {
    console.error(
      `❌ Supervisor Notification (AR): FAIL. Got: ${supervisorNotifAr?.title}`,
    );
  }

  if (promoterNotifAr && promoterNotifAr.title === "تم تسجيل الخروج بنجاح") {
    console.log("✅ Promoter Notification (AR): OK");
  } else {
    console.error(
      `❌ Promoter Notification (AR): FAIL. Got: ${promoterNotifAr?.title}`,
    );
  }
