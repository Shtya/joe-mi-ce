...
export class ProjectService extends BaseService<Project> {
  ...
  async find(userId: any) {
    if (
      userId?.role.name != ERole.SUPERVISOR &&
      userId?.role?.name != ERole.PROJECT_ADMIN
    ) {
  ...
  async resetProjectPlans(projectId: string): Promise<any> {
    ...
    console.log(
      `Deleted all journeys for project ${projectId} on date ${todayStr}`,
    );
    ...
    const [savedShift1, savedShift2] = await this.shiftRepo.save([
      shift1,
      shift2,
    ]);
    ...
    const historicalAssociations = await this.journeyRepo
    ...
    console.log(
      `Resetting plans for project ${projectId}. Found ${users.length} users, ${branches.length} branches, and ${historicalAssociations.length} PLANNED associations from yesterday (${yesterdayStr}).`,
    );
    ...
    for (const user of users) {
      ...
      const supervised = branches.filter(
        (b) =>
      ...
      if (branchIds.size === 0) {
        console.warn(
          `User ${user.id} (${user.username}) skipped: No associated branch found.`,
        );
        continue;
      }
    ...
    if (todayJourneys.length > 0) {
      ...
      console.log(
        `Generated ${todayJourneys.length} journeys for today (${todayStr})`,
      );
    }
