...
export class AuthController {
  ...
  async getUsers(@Query() query: any, @Req() req: any) {
    ...
    return CRUD.findAll(
      ...
      query.sortBy,
      query.sortOrder,
      ["role", "project", "branch", "created_by"],
  ...
  async importPromoters(
    ...
    try {
      ...
      console.log(
        `Promoter import completed: Success: ${result.success}, Failed: ${result.failed}`,
      );
      return result;
    } catch (err) {
      if (fs.existsSync(filePath)) {
  ...
  private async processBatch(
    ...
    try {
      ...
      console.log(
        `Promoter batch processed: ${batch.length} rows. Progress: ${result.processed}/${result.totalRows || "?"}`,
      );
