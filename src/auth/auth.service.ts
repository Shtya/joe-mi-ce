...
export class AuthService {
  ...
  async register(
    ...
    if (requester) {
      if (
        dto.role === ERole.SUPER_ADMIN &&
        requester.role.name !== ERole.SUPER_ADMIN
      ) {
      ...
      if (requester.role.name === ERole.PROJECT_ADMIN) {
        ...
        // }
        dto.project_id = await this.userService.resolveProjectIdFromUser(
          requester.id,
        );
      }
    } else {
    ...
    if (
      dto.role === ERole.PROJECT_ADMIN &&
      requester?.role.name === ERole.SUPER_ADMIN
    ) {
      if (!dto.project_name) {
        throw new BadRequestException(
          "Project name is required when creating Project Admin",
        );
      }
      ...
      const projectId =
        requester?.role.name === ERole.PROJECT_ADMIN
          ? requester.project?.id || requester.project_id
          : dto.project_id;
      if (!projectId) {
        throw new BadRequestException("Project ID is required for this role");
      }
    ...
    if (dto.branch_id) {
      ...
      if (!branch) {
        throw new BadRequestException(
          "Branch not found or not part of the project",
        );
      }
      ...
      if (dto.role === ERole.SUPERVISOR) {
        ...
        const existingSupervisor = projectBranches.find(
          (b) => b.supervisor?.username === dto.username,
        );
        if (existingSupervisor) {
          throw new ConflictException(
            "This supervisor is already assigned to another branch",
          );
        }
      }
      ...
      if (dto.role === ERole.PROMOTER) {
        const promoterUsed = projectBranches.some((b) =>
          b.team?.some((user) => user.username === dto.username),
        );
        if (promoterUsed) {
          throw new ConflictException(
            "This promoter is already assigned to a branch in this project",
          );
        }
      }
    }
  ...
  async getUserById(userId: string) {
    ...
    const project = await this.projectRepository.findOneBy({
      id: userWithRelations.project_id,
    });
  ...
  async importPromoters(rows: any[], requester: User) {
    ...
    for (const [index, rawRow] of rows.entries()) {
      try {
        ...
        console.log(row);
        await this.importSinglePromoter(
          row,
          requester,
          requesterProjectId,
          promoterRole,
  ...
  private async downloadAndSaveImage(imageUrl: string): Promise<string> {
    ...
    try {
      ...
      console.error(
        `Error downloading avatar from ${imageUrl}:`,
        error.message,
      );
      return null;
    }
