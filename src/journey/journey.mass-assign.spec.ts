...
describe("JourneyService - mass assign shift", () => {
  ...
  it("should throw NotFoundException if shift not found", async () => {
    ...
    await expect(
      service.assignShiftToAllPromoters(
        { projectId: "p1", shiftId: "s1" },
        {} as any,
      ),
  ...
  it("should assign shift by clearing old days instead of deleting", async () => {
    ...
    mockRepo.manager.createQueryBuilder = jest
      .fn()
      .mockReturnValue(mockQueryBuilder);

    const result = await service.assignShiftToAllPromoters(
      { projectId: "pr1", shiftId: "s1" },
      { id: "admin1" } as any,
    );
    ...
    expect(mockRepo.manager.create).toHaveBeenCalledWith(
      JourneyPlan,
