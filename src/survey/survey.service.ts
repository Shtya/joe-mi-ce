...
export class SurveyService {
  constructor(
    ...
    public feedbackRepo: Repository<SurveyFeedback>,
    @InjectRepository(SurveyFeedbackAnswer)
    public answerRepo: Repository<SurveyFeedbackAnswer>,
    @InjectRepository(SurveyQuestion)
    public questionRepo: Repository<SurveyQuestion>,
    ...
    dto.questions?.forEach((q) => {
      if (
        ...
        throw new BadRequestException(
          `Question "${q.text}" requires non-empty options for DROPDOWN type`,
        );
      }
  ...
  async createFeedback(dto: CreateSurveyFeedbackDto) {
    ...
    for (const ans of dto.answers) {
      ...
      if (!q) {
        throw new BadRequestException(
          `Question ${ans.questionId} is not part of this survey`,
        );
      }
      ...
      if (q.type === SurveyQuestionType.DROPDOWN) {
        if (!q.options || q.options.length === 0) {
          throw new BadRequestException(
            `Question "${q.text}" has no options configured`,
          );
        }
        if (!q.options.includes(val)) {
          throw new BadRequestException(
            `Invalid answer for "${q.text}". Expected one of: ${q.options.join(", ")}`,
          );
        }
      }
      ...
      if (q.type === SurveyQuestionType.IMAGE) {
        const isHttpUrl =
          typeof val === "string" && /^https?:\/\/\S+/i.test(val);
        const isDataUri =
          typeof val === "string" &&
          /^data:image\/[a-zA-Z0-9.+-]+;base64,/.test(val);
        if (!isHttpUrl && !isDataUri) {
          throw new BadRequestException(
            `Invalid image answer for "${q.text}". Provide a public URL (http/https) or data:image Base64.`,
          );
        }
      }
    }
  ...
  async remove(id: string, projectId: string): Promise<void> {
    ...
    if (!survey) {
      throw new NotFoundException("Survey not found in this project");
    }
