...
export class PermissionsService extends BaseService<Permission> {
  ...
  async create(dto: CreatePermissionDto) {
    ...
    if (existing) {
      throw new BadRequestException(
        this.i18n.t("events.permisstions.errors.permission_name_exists"),
      );
    }
  ...
  async bulkCreate(dto: BulkCreatePermissionDto): Promise<{
    ...
    const existingPermissions = await this.permissionRepository
      ...
      .where("LOWER(permission.name) IN (:...names)", {
  ...
  async update(id: any, dto: UpdatePermissionDto) {
    const permission = await this.permissionRepository.findOne({
      where: { id },
    });
    if (!permission) {
      throw new NotFoundException(
        this.i18n.t("events.permisstions.errors.permission_not_found"),
      );
    }
